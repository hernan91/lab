<?php
	...
	function api_internal_products_modifyProduct($code, $name, $manufacturer, $category_code, $price, $state, $stock, $description){
		$errors = validations_products_validModifyProduct($code, $name, $manufacturer, $category_code, $price, $state, $stock, $description);
		if(count($errors)>0) return $errors;
		$con = new Conexion();
		if($con->connect()){
			if(empty($code)) $query = "UPDATE `products` SET `name`='".$name."', `manufacturer`='".$manufacturer."', `category_code`='".$category_code."',`price`='".$price."',`state`='".$state."',`stock`='".$stock."',`description`='".$description."' WHERE `code`='".$code."'"; 
			else $query = "UPDATE `products` SET `code`='".$code."',`name`='".$name."', `manufacturer`='".$manufacturer."', `category_code`='".$category_code."',`price`='".$price."',`state`='".$state."',`stock`='".$stock."',`description`='".$description."' WHERE `code`='".$code."'";
			$result = $con->query($query);
			if($result) return $result;
